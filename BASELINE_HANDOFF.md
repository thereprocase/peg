# Rectangular baseline — historical handoff

> Historical snapshot. The current round design and active instructions are in [HANDOFF.md](HANDOFF.md).

## Latest geometry requirement

The user explicitly directed: “square corners will crush hole and provide bad bearing and unsnug fit. holes are round. make pegs round.” This archived instruction initiated the round revision. That work is now complete and promoted to the canonical root `round_*` sources; see [HANDOFF.md](HANDOFF.md) for current geometry, evidence, and continuation. A small edge fillet does not meet this requirement. Round pegs still require deliberate clearance and seated retention; circular geometry alone does not eliminate play.
