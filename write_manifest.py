...
manifest={'schema_version':1,'project':'thereprocase/peg','scope':'All intended tracked release files, excluding this manifest, Git metadata, caches, and review-only G-code. Current conformal files and labeled round and rectangular predecessors are preserved.','hash_algorithm':'SHA-256','file_count':len(files),'total_size_bytes':sum(f['size_bytes'] for f in files),'files':files}
